    ...
    <div className="flex min-h-screen">
      <aside className="w-64 bg-gray-900 text-white p-6 space-y-4">
        ...
        <nav className="space-y-2">
          <button onClick={() => setSection("dashboard")} className="block hover:text-amber-400">📊 Dashboard</button>
          <div>
            <button onClick={() => setOpenStats(!openStats)} className="block w-full text-left hover:text-amber-400">
          ...
          <button onClick={() => setSection("cars")} className="block hover:text-amber-400">🚙 Mașini</button>
          <button onClick={() => setSection("orders")} className="block hover:text-amber-400">📄 Comenzi</button>
          <button onClick={() => setSection("users")} className="block hover:text-amber-400">👥 Utilizatori</button>
        </nav>
      </aside>

      <main className="flex-1 p-8 bg-gray-50 overflow-y-auto">{renderSection()}</main>
    </div>
  );
}
